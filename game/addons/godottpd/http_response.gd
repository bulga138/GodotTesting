...
## GTD-003 deviation patch: per-client Mutex handed over by HttpServer; all
## socket writes below are serialized against the main thread's reads of the
## same peer. Null when the response is used standalone (upstream behavior).
var client_lock: Mutex = null
...
func send_raw(status_code: int, data: PackedByteArray = PackedByteArray([]), content_type: String = "application/octet-stream", extra_header: String = "") -> void:
	# GTD-003 deviation patch: the whole write is one critical section so the
	# response cannot interleave with main-thread reads of the same peer.
	if client_lock:
		client_lock.lock()
...
func send_partial(status_code: int, data: PackedByteArray = PackedByteArray([]), content_type: String = "application/octet-stream", extra_header: String = "") -> void:
	# GTD-003 deviation patch: guard partial writes too.
	if client_lock:
		client_lock.lock()
	client.put_data(data)
	if client_lock:
